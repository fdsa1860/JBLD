function action_SubJHMDB(data, tr_info, labels, opt)

% jointsVel = getVelocity(data.joints, 1);
...
tr_te_splits = tr_info.tr_te_splits;
n_tr_te_splits = size(tr_te_splits, 1);
...
for si = 1:n_tr_te_splits
    ...
    tr_ind = (tr_te_splits(si, :)==1);
    te_ind = (tr_te_splits(si, :)==2);
...
%     [predicted_labels,W,time] = nn_sos2(X_train, y_train, X_test, opt);
    [predicted_labels, time] = BagOfWords(X_train, y_train, X_test, opt);
